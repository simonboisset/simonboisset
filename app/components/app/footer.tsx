...
import { Linkedin, Mail, PaintBucket } from "lucide-react";
...
export const Footer = () => {
  ...
  const isDefaultLanguage = !lang || lang === DEFAULT_LANGUAGE;
  const prefix = isDefaultLanguage ? "" : `/${lang}`;
  return (
    <footer className="w-full p-4 mt-12 bg-gradient-to-t from-primary/25 to-transparent pt-24">
      <div className="flex sm:flex-row gap-8 flex-col justify-between items-center sm:items-start w-full max-w-screen-lg mx-auto">
        <div className="flex flex-col gap-2 flex-[2] sm:items-start items-center">
          ...
          <Link to={`${prefix}`} className="text-primary">
            {t((l) => l.footer.home)}
          </Link>
          <Link to={`${prefix}/blog`} className="text-primary">
            {t((l) => l.footer.blog)}
          </Link>
        </div>
        <div className="flex flex-col flex-[2] sm:items-center items-center">
          <div className="flex flex-col gap-2  sm:items-start items-center">
            <span className="text-lg font-bold">{t((l) => l.footer.docs)}</span>
            <Link to={`${prefix}/docs/legal`} className="text-primary">
              {t((l) => l.footer.legal)}
            </Link>
            <Link to={`${prefix}/docs/privacy`} className="text-primary">
              {t((l) => l.footer.privacy)}
            </Link>
          </div>
        </div>
        <div className="flex flex-col flex-[2] sm:items-end items-center">
          <div className="flex flex-col gap-2  sm:items-start items-center">
            ...
            <Link
              to="https://www.malt.fr/profile/simonboisset"
              className="text-primary flex flex-row items-center gap-2"
            >
            ...
            <Link
              to="https://twitter.com/simonboisset"
              className="text-primary flex flex-row items-center gap-2"
            >
